...
Steady-state resource rules for the backend: the producer park cap (duplicated in `output/mod.rs` and `player/mixer.rs`) IS the wakeup cadence, because rings run full during playback — do not shrink it casually, and change both copies together. Default-device polling uses an adaptive stride (`output_runtime`) because the probe enumerates devices on hosts without a platform default-device id (per-device server roundtrips on PulseAudio). Linux shares Android's latency profile (deep 48-block software queues plus start/seek prebuffer watermarks, cfg `any(android, linux)`) but not its device-buffer policy: CPAL's PulseAudio backend maps `BufferSize::Default` onto a server-chosen `BufferAttr` that pipewire-pulse inflates to ~2s, so `output/platform/linux.rs` requests an explicit period instead. Keep that period derived from milliseconds rather than a frame constant (CPAL doubles it into the Pulse target length, so a frame constant shrinks the latency target on high-rate sinks), and keep the `Unknown` supported-range arm on `BufferSize::Default` — that is the ALSA path, which has no Pulse buffer to fix and can fail stream creation on a forced period. Otherwise the `stable_buffer_size` device-buffer policy must not be changed without a concrete platform reason.
