...
//!                                     JS audioWs.ts
//!
//!  JS sends command ──► per-connection read loop ──► PlayerHandle.send(msg)
//! ```
//!
//! Inbound messages are JSON `AudioThreadEventMessage<AudioThreadMessage>`.
//! Outbound messages are JSON `AudioThreadEventMessage<AudioThreadEvent>`.
...
pub struct WsServer {
  /// Loopback address we ended up binding to (port chosen by the OS).
  bound_addr: SocketAddr,
  /// Outbound event broadcast — every connection gets its own subscriber.
  event_tx: broadcast::Sender<String>,
  /// Keep the listener task alive for the lifetime of the server.
  _listener_task: tokio::task::JoinHandle<()>,
}

impl WsServer {
  /// Bind to `127.0.0.1:0` (kernel picks port). Returns once the listener
  /// is accepting connections — callers can immediately read `ws_url()`.
  pub async fn start(player_handle: PlayerHandle) -> anyhow::Result<Self> {
    let listener = TcpListener::bind(("127.0.0.1", 0)).await?;
    let bound_addr = listener.local_addr()?;
    info!("音频 WebSocket 服务器已开启: ws://{bound_addr}");
    ...
    let listener_task = tokio::spawn(async move {
      loop {
        match listener.accept().await {
          Ok((stream, peer)) => {
            debug!("接受 WebSocket 连接: {peer}");
            let event_rx = event_tx_for_task.subscribe();
            let player = player_handle.clone();
            tokio::spawn(handle_connection(stream, peer, event_rx, player));
          }
          Err(e) => {
            warn!("WebSocket accept 出错: {e:?}");
  ...
  /// clients are connected (or the channel is full) the message is dropped
  /// silently — the audio thread must never block on the network.
  pub fn broadcast_event(&self, evt: &AudioThreadEventMessage<AudioThreadEvent>) {
    ...
    // it means the frontend isn't connected yet; events will resume once
    // it is. The Tauri-emit fallback (in player/mod.rs) keeps state in
...
async fn handle_connection(
  ...
  // Inbound: parse JSON → dispatch to player.
